...
#define BGR(color)                                     \
  ((((color) & 0xFF) << 16) | ((color) & 0xFF00FF00) | \
   (((color) & 0xFF0000) >> 16))
...
void setfillstyle(int style, long hatch = HS_DIAGCROSS,
                  const IMAGE* ppattern = NULL);  // Set fill style
...
// adaptive. 		lpszFace: The font name 		nEscapement: The
// writing angle of the string, 0.1 degrees, defaults to 0.
// nOrientation: The writing angle of each character, 0.1 degrees, defaults to
// 0. 		nWeight: The stroke weight of the character 		bItalic:
// Specify whether the font is italic 		bUnderline: Specify whether the
// font is underlined 		bStrikeOut: Specify whether the font has a
// strikeout 		fbCharSet: Specifies the character set
// fbOutPrecision: Specifies the output accuracy of the text
// fbClipPrecision: Specifies the clip accuracy of the text
// fbQuality: Specifies the output quality of the text fbPitchAndFamily:
...
int loadimage(IMAGE* pDstImg, LPCTSTR pImgFile, int nWidth = 0, int nHeight = 0,
              bool bResize = false);  // Load image from a file
                                      // (bmp/gif/jpg/png/tif/emf/wmf/ico)
int loadimage(IMAGE* pDstImg, LPCTSTR pResType, LPCTSTR pResName,
              int nWidth = 0, int nHeight = 0,
              bool bResize = false);  // Load image from resources
                                      // (bmp/gif/jpg/png/tif/emf/wmf/ico)
void saveimage(
    LPCTSTR pImgFile,
    const IMAGE* pImg = NULL);  // Save image to a file (bmp/gif/jpg/png/tif)
...
void putimage(int dstX, int dstY, int dstWidth, int dstHeight,
              ...
              DWORD dwRop = SRCCOPY);  // Put image to device
void rotateimage(IMAGE* dstimg, const IMAGE* srcimg, double radian,
...
DWORD* GetImageBuffer(
    const IMAGE* pImg = NULL);  // Get the display buffer of the graphics device
...
// pressed 				WM_LBUTTONUP		Left mouse
// button is released 				WM_LBUTTONDBLCLK Left mouse
// button is double-clicked 				WM_MBUTTONDOWN
// Middle mouse button is pressed 				WM_MBUTTONUP
// Middle mouse button is released 				WM_MBUTTONDBLCLK
// Middle mouse button is double-clicked
// WM_RBUTTONDOWN		Right mouse button is pressed
// WM_RBUTTONUP		Right mouse button is released WM_RBUTTONDBLCLK	Right
...
//	EX_CHAR		WM_CHAR
//
//	EX_WINDOW	WM_ACTIVATE			The window is activated
// or deactivated 				WM_MOVE
// The window has been moved 				WM_SIZE
...
struct ExMessage {
  ...
  union {
    ...
    struct {
      ...
      short wheel;  // The distance the wheel is rotated, expressed in multiples
                    // or divisions of 120
    };
    ...
    struct {
      ...
      bool extended
          : 1;  // Indicates whether the key is an extended key, such as a
                // function key or a key on the numeric keypad. The value is
...
void setcapture();  // Enable the ability to capture mouse messages outside of
                    // the graphics window
void releasecapture();  // Disable the ability to capture mouse messages outside
