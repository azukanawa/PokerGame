 ...
 *		The functions and constants declared in this file are only for
...
#define DIAGCROSS_FILL \
  BS_HATCHED, HS_DIAGCROSS  // Fill with xxx (heavy cross hatch fill).
...
#define _EASYX_DEPRECATE_WITHNEW(_NewFunc)                            \
  __declspec(deprecated(                                              \
      "This function is deprecated. Instead, use this new "           \
      "function: " #_NewFunc ". See https://docs.easyx.cn/" #_NewFunc \
      " for details."))
#define _EASYX_DEPRECATE_OVERLOAD(_Func)                               \
  __declspec(deprecated(                                               \
      "This overload is deprecated. See https://docs.easyx.cn/" #_Func \
      " for details."))
#else
...
_EASYX_DEPRECATE_WITHNEW(settextstyle)
void setfont(int nHeight, int nWidth, LPCTSTR lpszFace, int nEscapement,
...
void bar3d(int left, int top, int right, int bottom, int depth,
           bool topflag);  // Draw a filled 3D rectangle with a border

void drawpoly(int numpoints,
              const int* polypoints);  // Draw a polygon without filling
void fillpoly(int numpoints,
              const int* polypoints);  // Draw a filled polygon with a border

int getmaxx();  // Get the maximum x-coordinate in the physical coordinates of
                // the graphics window
int getmaxy();  // Get the maximum y-coordinate in the physical coordinates of
                // the graphics window
...
_EASYX_DEPRECATE void outtext(
    LPCTSTR str);                        // Output a string at current location
...
struct MOUSEMSG {
  ...
  short wheel;  // The distance the wheel is rotated, expressed in multiples or
                // divisions of 120
};
_EASYX_DEPRECATE bool MouseHit();  // Indicates whether there are mouse messages
_EASYX_DEPRECATE_WITHNEW(getmessage)
MOUSEMSG GetMouseMsg();  // Get a mouse message. if mouse message queue is
                         // empty, wait.
_EASYX_DEPRECATE_WITHNEW(peekmessage)
bool PeekMouseMsg(
    MOUSEMSG* pMsg,
    bool bRemoveMsg = true);  // Get a mouse message and return immediately
